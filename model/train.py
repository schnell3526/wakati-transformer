...
def main():
    ...
    def tokenize_and_align_labels(examples):
        ...
        labels = []
        for input_ids, label in zip(tokenized_inputs['input_ids'], examples[label_column_name]):
            if len(label) <= 126:
                label_ids = [-100] + [label_to_id[l] for l in label] + [-100]*(126-len(label)+1)
            else:
                label_ids = [-100] + [label_to_id[label[idx]] for idx in range(126)] + [-100]
